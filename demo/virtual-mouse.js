...
export function createVirtualMousePlayer(options = {}) {
    // --------------------------------------------------------------------------- INIT
    // Default options
    ...
    let reactPropsKey;
    // --------------------------------------------------------------------------- PRIVATES
    function log(method, object, element) {
        if (verbose) {
            console.info(`${method} - ${JSON.stringify(object)}`);
            element && console.log(element);
        }
    }
    function dispatchSyntheticEvent(element, property, event) {
    ...
    function reactSyntheticEvent(element, event) {
        if (!reactPropsKey)
            return;
        ...
        if (event.bubbles) {
            ...
            elements.forEach(el => dispatchSyntheticEvent(el, property, event));
        }
        else {
    ...
    function createMouseEvent(type, bubbles = true) {
        const { x, y } = position;
    ...
    function getParentElementsSet(element) {
    ...
    return {
        ...
        async scrollTo(x, y, options, element) {
        ...
        async scroll(x, y, options, element) {
            element ??= getHoveredElement();
            const scrollableElement = getScrollableParent(element);
